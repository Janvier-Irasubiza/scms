...
const RehabDashboard = () => {
  ...
  const uri = "In rehabilitation center";
  const [endpoint, setEndpoint] = useState(
    `http://127.0.0.1:8000/api/stat-children/?status=${encodeURIComponent(uri)}`
  );
  ...
  const [chlildStatusCount, setStatusCountData] = useState<{
    in_rehab: {
      count: number;
    };
    rehabilitated: {
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/api/rehab-overview/`, {
        method: "GET",
        headers: {
            ...
            <div className="w-full">
              <div className="d-flex justify-content-between gap-4">
                <div className="w-full smry-card rounded">
                  <p className="m-0">In rehabilitation</p>
                  <h2 className="m-0">{childrenData.length}</h2>
                </div>
                <div className="w-full smry-card rounded">
                  <p className="m-0">Rehabilitated</p>
                  <h2 className="m-0">
                    {chlildStatusCount?.rehabilitated.count}
                  </h2>
                  {chlildStatusCount?.rehabilitated.count != 0 ? (
                    <Link to="/rehab/rehabilitated">Review</Link>
                  ) : (
                    <></>
                  )}
                </div>
